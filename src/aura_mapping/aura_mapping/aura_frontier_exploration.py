...
config_path = os.path.join(get_package_share_directory("aura_mapping"), "config", "exploration_params.yaml")
...
class navigationControl(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Odometry,'/aura_controller/odom',self.odom_callback,10)
        self.subscription = self.create_subscription(LaserScan,'scan',self.scan_callback,10)
        self.publisher = self.create_publisher(Twist, '/aura_controller/cmd_vel_unstamped', 10)
        print("[INFO] IN EXPLORATION MODE")
        self.kesif = True
